...
RSpec.describe RecipeIngredient, type: :model do
  ...
  describe "class methods" do
    ...
    it "find all ingredients for a recipe" do
      expected_1 = [@rec_ing_1, @rec_ing_2, @rec_ing_3, @rec_ing_4, @rec_ing_5, @rec_ing_6]
      expected_2 = [@rec_ing_7, @rec_ing_8, @rec_ing_9, @rec_ing_10, @rec_ing_11]
      expected_3 = [@rec_ing_12, @rec_ing_13, @rec_ing_14, @rec_ing_15, @rec_ing_16]

      expect(RecipeIngredient.find_ingredients(@recipe_1.id)).to eq(expected_1)
      expect(RecipeIngredient.find_ingredients(@recipe_2.id)).to eq(expected_2)
      expect(RecipeIngredient.find_ingredients(@recipe_3.id)).to eq(expected_3)
    end
  end
end